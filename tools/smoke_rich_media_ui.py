...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.get_by_role('button', name='字体颜色', exact=True).click()
        page.get_by_role('button', name='字体颜色 #b42318', exact=True).click()
        expect(rich.locator('.rt-fg-red').first).to_be_visible()
        page.get_by_role('button', name='字体颜色', exact=True).click()
        page.get_by_label('字体颜色 HEX 色号', exact=True).fill('#zzzzzz')
        page.get_by_label('字体颜色 HEX 色号', exact=True).press('Enter')
        expect(page.locator('#font-color-panel .form-error')).not_to_be_empty()
        page.get_by_label('字体颜色调色盘', exact=True).fill('#3467ab')
        page.locator('#font-color-panel').get_by_role('button', name='应用', exact=True).click()
        page.get_by_role('button', name='文字背景色', exact=True).click()
        page.get_by_label('文字背景色 HEX 色号', exact=True).fill('#e1f4d8')
        page.get_by_label('文字背景色 HEX 色号', exact=True).press('Enter')
        page.get_by_label('字号', exact=True).select_option('24px')
        page.get_by_role('button', name='加粗', exact=True).click()
        expect(rich.locator('.rt-fg-3467ab').first).to_be_visible()
        assert rich.locator('.rt-fg-3467ab').first.evaluate('(el) => getComputedStyle(el).color') == 'rgb(52, 103, 171)'
        ...
        page.reload()
        expect(rich.locator('.rt-fg-3467ab').first).to_be_visible()
        expect(rich.locator('.rt-bg-e1f4d8').first).to_be_visible()
        assert rich.locator('.rt-bg-e1f4d8').first.evaluate('(el) => getComputedStyle(el).backgroundColor') == 'rgb(225, 244, 216)'
        ...
        page.get_by_role('button', name='字体颜色', exact=True).click()
        box = page.locator('#font-color-panel').bounding_box()
        assert box['x'] >= 0 and box['x'] + box['width'] <= 390
        assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
        page.screenshot(path=str(folder / 'rich-media-mobile.png'), full_page=True)
        page.get_by_role('button', name='字体颜色', exact=True).press('Escape')
        expect(page.locator('#font-color-panel')).to_be_hidden()
        page.get_by_role('button', name='发布帖子', exact=True).click()
        page.wait_for_url(BASE + '/posts/' + post_id)
        expect(page.locator('.article-body .rt-fg-3467ab').first).to_be_visible()
        assert page.locator('.article-body .rt-fg-3467ab').first.evaluate('(el) => getComputedStyle(el).color') == 'rgb(52, 103, 171)'
        ...
        expect(page.locator('details').first.locator('.rt-fg-3467ab').first).to_be_visible()
