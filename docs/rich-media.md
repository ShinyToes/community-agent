...
2. 选中文字后设置加粗、斜体、下划线、删除线、字体颜色、背景色和字号。「字色」「底色」展开40色色块面板；可以直接点色块，或点击「自选颜色」打开系统调色盘，也可输入六位HEX色号（例如 #3478ab）并点击「应用」。支持恢复默认字色、清除背景色，Esc或点击面板外关闭。字号为14/16/20/24/32。
...
服务端只生成允许的HTML标签和CSS类；颜色严格校验为六位HEX，规范化后生成对应颜色规则。每个HTML响应使用独立nonce授权颜色样式表，编辑器向同一授权样式表添加规则；CSP不启用unsafe-inline，不接收任意style/script或图片URL。富文本前端已不依赖实验性Markdown转换层。
...
2026-09-17 自选颜色更新：SQLite完整56项通过；浏览器验证预设色、系统调色控件、HEX输入校验、草稿重载、发布页实际颜色、历史版本与390px手机面板。此次无数据库迁移。
